...
import { resolveWorkspaceRef, resolveConversationId } from '../lib/refs.js'
...
async function viewConversation(ref: string, options: ViewOptions): Promise<void> {
  ...
  if (options.ndjson) {
    ...
    for (const m of messages) {
      console.log(JSON.stringify({ type: 'message', ...m, creatorName: userMap.get(m.creator) }))
    }
    return
  }

  const participants = conversation.userIds.map((id) => userMap.get(id) || `user:${id}`).join(', ')
