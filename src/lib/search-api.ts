...
export async function extendedSearch(params: ExtendedSearchParams): Promise<ExtendedSearchResponse> {
  ...
  const items = (data.items || []).map((item: Record<string, unknown>) => transformSearchResult(item))
  return {
    items,
