...
- `npm run lint`: Auto-fixes lint and formatting issues
...
3. **Automatic pre-release:** Each PR merge into `next` triggers CI and publishes a pre-release version (e.g., `1.0.0-next.1`) to the `@next` dist-tag on npm.
